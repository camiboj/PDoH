...
public class DnsDownWorker implements Runnable {
    ...
    private void sendDnsPack(UdpTunnel tunnel, byte[] data) throws IOException {
            ...
            while (true) {
                ...
                while (keyIterator.hasNext()) {
                    ...
                    if (key.isValid() && key.isReadable()) {
                        try {
                            ByteBuffer receiveBuffer = dnsResponsesQueue.take();
                            receiveBuffer.flip();
                            byte[] data = new byte[receiveBuffer.remaining()];
                            receiveBuffer.get(data);
                            sendDnsPack((UdpTunnel) key.attachment(), data);
                        } catch (IOException e) {
                            Log.e(TAG, "error", e);
                        }
                    }
